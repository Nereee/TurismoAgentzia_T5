DROP DATABASE IF EXISTS db_E2T5;
CREATE DATABASE IF NOT EXISTS db_E2T5
charset utf8mb4
...
CREATE TABLE bidaia (
    idBid int PRIMARY KEY NOT NULL,
    izena varchar(20),
    desk text,
    hasieraData date,
    amaieraData date,
...
CREATE TABLE zerbitzuak (
    idZerb int PRIMARY KEY not null,
    idBid int,
...
CREATE TABLE hegaldia (
    hegaldi_kodea VARCHAR(50),
    kodAeroportu_jatorria char(3),
    kodAeroportu_itzulera char(3),
    idZerb int primary key,
    FOREIGN KEY (idZerb) REFERENCES zerbitzuak (idZerb) ON DELETE CASCADE,
    prezioa DECIMAL,
    irteera_data DATE,
    ...
    KodAirelinea char(5),
    FOREIGN KEY (KodAirelinea) REFERENCES airelineak (KodAirelinea) ON DELETE CASCADE,
    KodAireportua char(3),
    FOREIGN KEY (KodAireportua) REFERENCES aireportua (kodAireportua) ON DELETE CASCADE
);
...
CREATE TABLE joan_etorri (
    ...
    irteera_data date,
    irteera_ordua time,
    PRIMARY KEY (idZerb, hegaldi_kodea)
);
use db_e2t5;
-- INSERTS FOR IATA
